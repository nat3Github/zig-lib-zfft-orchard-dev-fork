...
pub fn mr_dit_bi_G_P(comptime C: type, w: [*]C, N: usize, in: [*]C, out: [*]C) void {
    ...
    while (j < log2_N) : (j += 2) {
        ...
        while (i < math.shl(usize, 1, s)) : (i += 1) {
            ...
            var k: usize = 1;
            while (k < l / 2) : (k += 1) {
                mr_dit_bf4(C, l, t + k, get_twiddle(C, math.shl(usize, k, s), log2_N, w), get_twiddle(C, math.shl(usize, k * 2, s), log2_N, w), get_twiddle_mr(C, math.shl(usize, k * 3, s), log2_N, w));
            }
            k += 1;
            while (k < l) : (k += 1) {
                mr_dit_bf4(C, l, t + k, get_twiddle(C, math.shl(usize, k, s), log2_N, w), get_twiddle(C, math.shl(usize, k * 2, s), log2_N, w), get_twiddle_mr(C, math.shl(usize, k * 3, s), log2_N, w));
