...
pub fn fft(comptime C: type, N: usize, w: [*]C, out: [*]C, in: [*]C) void {
    ...
    mr_dit_dr(C, w, in, out, 0, log2_N);
}

pub fn mr_dit_dr(comptime C: type, w: [*]C, in: [*]C, out: [*]C, s: usize, log2_N: usize) void {
    ...
    switch (l) {
        ...
        4 => {
            ...
            mr_dit_dr(C, w, in, out, s + 2, log2_N);
            mr_dit_dr(C, w, in + stride, out + l, s + 2, log2_N);
            mr_dit_dr(C, w, in + stride * 2, out + l * 2, s + 2, log2_N);
            mr_dit_dr(C, w, in + stride * 3, out + l * 3, s + 2, log2_N);
