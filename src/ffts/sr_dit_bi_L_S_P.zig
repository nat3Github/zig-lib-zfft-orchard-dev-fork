...
pub fn fft(comptime C: type, N: usize, w1: [*]C, w3: [*]C, out: [*]C, in: [*]C, sr_input_lut: [*]usize, sr_sched_off: [*]usize, sr_sched_cnt: [*]usize) void {
    ...
    sr_dit_bi_L_S_P(C, w1, w3, out, in, N, log2_N, sr_input_lut, sr_sched_off, sr_sched_cnt);
}

pub fn sr_dit_bi_L_S_P(comptime C: type, w1: [*]C, w3: [*]C, out: [*]C, in: [*]C, N: usize, log2_N: usize, sr_input_lut: [*]usize, sr_sched_off: [*]usize, sr_sched_cnt: [*]usize) void {
    ...
    if (log2_N > 3) {
        ...
        while (s < log2_N) : (s += 1) {
            ...
            while (i < c) : (i += 1) {
                ...
                var k: usize = 1;
                while (k < os / 2) : (k += 1) {
                    const tw1: C = get_twiddle(C, math.shl(usize, k, t), log2_N, w1);
                    const tw3: C = get_twiddle(C, math.shl(usize, k, t), log2_N, w3);

                    sr_dit_bf4(C, os, o + k, tw1, tw3);
                    sr_dit_bf4(C, os, o + os - k, conjI(tw1), neg(conjI(tw3)));
                }
            }
        }
    }
