...
pub fn fft(comptime C: type, N: usize, w: [*]C, out: [*]C, in: [*]C, cp_input_lut: [*]usize, sr_sched_off: [*]usize, sr_sched_cnt: [*]usize) void {
    cp_dit_bi_G_L_S_P(C, w, N, in, out, cp_input_lut, sr_sched_off, sr_sched_cnt);
}

pub fn cp_dit_bi_G_L_S_P(comptime C: type, w: [*]C, N: usize, in: [*]C, out: [*]C, cp_input_lut: [*]usize, sr_sched_off: [*]usize, sr_sched_cnt: [*]usize) void {
        ...
        while (s < log2_N) : (s += 1) {
            ...
            while (k < os / 2) : (k += 1) {
                ...
                while (i < c) : (i += 1) {
                    ...
                    cp_dit_bf(C, os, out + o + os - k, negIConj(tw));
                }
            }
        }
    }
