...
pub fn fft(comptime C: type, N: usize, w: [*]C, w_sr: [*]C, out: [*]C, in: [*]C) void {
    ...
    sr_dit_dr_S_P(C, w, w_sr, log2_N, in, out, log2_N, 0);
}

pub fn sr_dit_dr_S_P(comptime C: type, w: [*]C, w_sr: [*]C, log2_N: usize, in: [*]C, out: [*]C, l: usize, i: usize) void {
    ...
    switch (l) {
        ...
        2 => {
            ...
            sr_dit_dr_S_P(C, w, w_sr, log2_N, in, out, l - 1, i);
            sr_dit_dr_S_P(C, w, w_sr, log2_N, in, out + os * 2, l - 2, i + is);
            sr_dit_dr_S_P(C, w, w_sr, log2_N, in, out + os * 3, l - 2, i + is * 3);
            ...
            var k: usize = 1;
            while (k < os / 2) : (k += 1) {
                const w1: C = get_twiddle(C, math.shl(usize, k, t), log2_N, w);
                const w3: C = get_twiddle_sr(C, math.shl(usize, k, t), log2_N, w_sr);

                sr_dit_bf4(C, os, out + k, w1, w3);
                sr_dit_bf4(C, os, out + os - k, conjI(w1), neg(conjI(w3)));
            }
        },
    }
}
